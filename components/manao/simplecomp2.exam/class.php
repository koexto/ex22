...
class SimpleComp2 extends CBitrixComponent
{
	...
	private function getProduction()
	{
		...
		$arNavParams = array(
			...
			"bShowAll" => true, // показывать ссылку Ђ¬се элементыї?
		);
		$arNavParams = false;
		...
		if (isset($_GET["F"]))
		{
			$arFilter[] = array(
				"LOGIC" => "OR",
				array("<=PROPERTY_PRICE" => 1700, "PROPERTY_MATERIAL" => "ƒерево, ткань"),
				array("<PROPERTY_PRICE" => 1500, "PROPERTY_MATERIAL" => "ћеталл, пластик"),
			);
		}
